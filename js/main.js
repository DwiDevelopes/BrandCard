...
// Tambahan: inject Font Awesome dan UI profesional (misal: Bootstrap) ke <head>
(function() {
    // Font Awesome
...
// Versi mobile-friendly, ukuran lebih kecil, dan tampilan lebih ringkas
...
CCTLMultiplayerGui.prototype.refreshRoomList = function(a) {
    ...
    for (var b = 0; b < a.length; b++) {
        c += '<li data-private="' + a[b]["private"] + '" data-accessible="' + a[b].accessible + '" class="list-group-item d-flex justify-content-between align-items-center p-2" style="font-size:14px;">';
        c += '<span class="' + this._cssClassDomain + 'room-name" style="font-size:15px;">';
        c += a[b].name;
        c += "</span>";
        c += '<span class="' + this._cssClassDomain + 'current-users badge bg-primary rounded-pill ms-2" style="font-size:12px;">';
        c += a[b].curusers + "/" + a[b].maxusers;
        c += "</span> ";
        c += !0 === a[b]["private"] ? '<i class="fas fa-lock text-danger ms-2"></i>' : !1 === a[b].accessible ? '<i class="fas fa-ban text-secondary ms-2"></i>' : '<i class="fas fa-sign-in-alt text-success ms-2"></i>';
        c += "</li>";
    }
    jQuery("." + this._cssClassDomain + "room-list").html(c)
};
CCTLMultiplayerGui.prototype.showRoomList = function(a) {
    var c = '<ul class="' + this._cssClassDomain + "list " + this._cssClassDomain + 'room-list list-group mb-2" style="max-height:220px;overflow:auto;padding:0 2px;">';
    c += '</ul><button onclick="on_ctl_multiplayer_refresh_room_list()" type="button" class="btn btn-secondary btn-sm ' + (this._cssClassDomain + "update " + this._cssClassDomain + 'btn-gray mb-2" style="font-size:13px;">');
    c += '<i class="fas fa-sync-alt"></i> ';
    c += "<span>" + TEXT_SYS_UPDATE + "</span>";
    this._idCurDialog = this.showDialog(TEXT_SYS_MATCH_LIST, c + "</button>", [{
        txt: '<i class="fas fa-bolt"></i> ' + TEXT_SYS_QUICKMATCH,
        cb: "on_ctl_multiplayer_join_quick_match",
        classes: "btn btn-primary btn-sm"
    }, {
        txt: '<i class="fas fa-plus"></i> ' + TEXT_SYS_CREATEMATCH,
        cb: "on_ctl_multiplayer_show_create_match",
        classes: "btn btn-success btn-sm"
    }, {
        txt: '<i class="fas fa-arrow-left"></i> ' + TEXT_SYS_BACK,
...
CCTLMultiplayerGui.prototype.showTypeRoomPassword = function(a) {
    var c = '<div class="' + this._cssClassDomain + 'form-group mb-2">';
    c += "<label class='form-label' style='font-size:13px;'>" + TEXT_SYS_TYPEROOMPASS + "</label>";
    this._idCurDialog = this.showDialog(TEXT_SYS_TYPEROOMPASS, c + ('<input type="password" class="form-control form-control-sm" name="password" data-room-name="' + a + '" style="font-size:13px;"></div>'), [{
        txt: '<i class="fas fa-check"></i> ' + TEXT_SYS_OK,
...
CCTLMultiplayerGui.prototype.showCreateRoom = function() {
    var a = '<div class="' + this._cssClassDomain + 'form-group mb-2">';
    a += "<label class='form-label' style='font-size:13px;'>" + TEXT_SYS_NAMEROOM + "</label>";
    a += '<input type="text" class="form-control form-control-sm" name="roomname" value="' + this._szNickName + "'s room\" style=\"font-size:13px;\">";
    a += '</div><div class="' + (this._cssClassDomain + 'form-group mb-2">');
    a += "<label class='form-label' style='font-size:13px;'>" + TEXT_SYS_PASSWORD + "</label>";
    a += '<input type="password" class="form-control form-control-sm" name="password" style="font-size:13px;"><p class="form-text" style="font-size:12px;">' + (TEXT_SYS_INFOPASS + "</p>");
    a += '</div><div class="' + (this._cssClassDomain + "form-group mb-2" + (2 === this._iMaxPlayersInRoom ? " " + this._cssClassDomain + "display-none" : "") + '">');
    a += "<label class='form-label' style='font-size:13px;'>" + TEXT_SYS_MAXPLAYERS + "</label>";
    a += '<ul class="' + this._cssClassDomain + 'inline-list list-inline mb-1">';
    if (2 < this._iMaxPlayersInRoom)
        for (var c = 2; c < this._iMaxPlayersInRoom + 1; c++)
            a += this._iStartNumPlayers === c ? ('<li class="list-inline-item"><input type="radio" name="maxplayers" value="' + c + '" checked="checked"><span style="font-size:13px;">' + c + "</span></li>") : ('<li class="list-inline-item"><input type="radio" name="maxplayers" value="' + c + '"><span style="font-size:13px;">' + c + "</span></li>");
    a += "<p class='form-text' style='font-size:12px;'>" + TEXT_SYS_CHOOSEMAXNUMPLAYERS + "</p>";
    this._idCurDialog = this.showDialog("Create room", a + "</div>", [{
        txt: '<i class="fas fa-plus"></i> ' + TEXT_SYS_CREATE,
...
CCTLMultiplayerGui.prototype.showChooseNickName = function() {
    this._idCurDialog = this.showDialog(TEXT_SYS_CHOOSENICK, '<input type="text" class="form-control form-control-sm" name="nickname" maxlength="20" value="' + this._szNickName + '" style="font-size:13px;">', [{
        txt: '<i class="fas fa-check"></i> ' + TEXT_SYS_OK,
        cb: "on_ctl_multiplayer_send_nickname",
        classes: "btn btn-primary btn-sm"
    }, {
        txt: '<i class="fas fa-times"></i> ' + TEXT_SYS_CLOSE,
        cb: "g_oCTLMultiplayer.closeCurrentDialog",
        classes: "btn btn-outline-secondary btn-sm"
    }])
};
...
CCTLMultiplayerGui.prototype.showDialog = function(a, c, b, e) {
    var d = "";
    e || (e = this.makeCode());
    d += "<div id='" + e + "' class='" + this._cssClassDomain + "dlg-wrapper modal show d-block' tabindex='-1' style='background:rgba(0,0,0,0.5);z-index:9999;'>";
    // d += "<div class='" + this._cssClassDomain + "dlg-block modal-dialog modal-dialog-centered' style='max-width:340px;width:96vw;'>";
    d += "<div class='" + this._cssClassDomain + "dlg-content modal-content' style='border-radius:10px;'>";
    d += "<div class='" + this._cssClassDomain + "dlg-header modal-header p-2' style='padding:6px 12px;'>";
    d += "<h1 class='modal-title fs-6' style='font-size:16px;'>" + a + "</h1></div>";
    d += "<div class='" + this._cssClassDomain + "dlg-content-body modal-body p-2' style='padding:10px 8px;'>";
    d += c;
    d += "</div>";
    if (b && 0 < b.length) {
        d += "<div class='" + this._cssClassDomain + "dlg-footer modal-footer p-2' style='padding:8px 8px;'>";
        for (a = 0; a < b.length; a++)
            d += "<button type='button' onclick='" + b[a].cb + '("' + e + "\");' class='" + this._cssClassDomain + "mini " + (b[a].classes || "") + "' style='font-size:13px;padding:4px 10px;margin:2px 2px;'>" + b[a].txt + "</button>";
...
CCTLMultiplayerGui.prototype.showLoading = function(a, c) {
    ...
    b += "<div id='" + this._idLoadingDialog + "' class='" + this._cssClassDomain + "dlg-wrapper " + this._cssClassDomain + "fixed modal show d-block' tabindex='-1' style='background:rgba(0,0,0,0.5);z-index:9999;'>";
    // b += "<div class='" + this._cssClassDomain + "dlg-block modal-dialog modal-dialog-centered' style='max-width:320px;width:96vw;'>";
    b += "<div class='" + this._cssClassDomain + "dlg-content modal-content' style='border-radius:10px;'>";
    b += "<div class='" + this._cssClassDomain + "dlg-header modal-header p-2'>";
    b += "<h1 class='modal-title fs-6' style='font-size:16px;'>" + a + "</h1></div>";
    b += "<div class='" + this._cssClassDomain + "dlg-content-body modal-body " + this._cssClassDomain + "align-center p-2' style='text-align:center;'>";
    b += '<i class="fas fa-spinner fa-spin fa-2x"></i>';
    b += "</div>";
    if (c) {
        b += "<div class='" + this._cssClassDomain + "dlg-footer modal-footer " + this._cssClassDomain + "center p-2'>";
        b += "<button type='button' onclick='" + c + '("' + this._idLoadingDialog + "\");' class='" + this._cssClassDomain + "mini btn btn-outline-secondary btn-sm' style='font-size:13px;'>" + TEXT_SYS_BACK + "</button>";
...
CCTLMultiplayerGui.prototype.updateRoomUserList = function(a, c) {
    ...
    if (0 < b.length) {
        ...
        for (var e = 0; e < a.length; e++)
            b.append("<li class='list-group-item p-1' style='font-size:13px;'>" + a[e] + "</li>");
        var info = TEXT_WAITING_ROOM_MESSAGE.replace("%d", c);
        jQuery("." + this._cssClassDomain + "room-info").text(info);
        var playNowBtn = jQuery("." + this._cssClassDomain + "room-play-now");
        if (playNowBtn.length) {
            if (a.length === 1)
                playNowBtn.addClass(this._cssClassDomain + "display-none");
            else
                playNowBtn.removeClass(this._cssClassDomain + "display-none");
        }
    }
};
CCTLMultiplayerGui.prototype.showWaitingPlayersInRoom = function(a, c, b) {
    ...
    this._idLoadingDialog = this.makeCode();
    a || (a = TEXT_SYS_LOADING);
    e += "<div id='" + this._idLoadingDialog + "' class='" + this._cssClassDomain + "dlg-wrapper " + this._cssClassDomain + "fixed modal show d-block' tabindex='-1' role='dialog' aria-hidden='true' style='z-index:9999;'>";
    // e += "<div class='" + this._cssClassDomain + "dlg-backdrop modal-backdrop fade show'></div>";
    // e += "<div class='" + this._cssClassDomain + "dlg-container modal-dialog-centered' style='max-width:320px;width:96vw;'>";
    e += "<div class='" + this._cssClassDomain + "dlg-content modal-content' style='border-radius:10px;'>";
    e += "<div class='" + this._cssClassDomain + "dlg-header modal-header p-2'>";
    e += "<h1 class='modal-title fs-6' style='font-size:16px;'>" + a + "</h1></div>";
    e += "<div class='" + this._cssClassDomain + "dlg-content-body modal-body " + this._cssClassDomain + "align-center p-2' style='text-align:center;'>";
    e += "<ul class='" + this._cssClassDomain + "list " + this._cssClassDomain + "user-list list-group mb-2' style='max-height:120px;overflow:auto;padding:0 2px;'>";
    for (a = 0; a < c.length; a++)
        e += "<li class='list-group-item p-1' style='font-size:13px;'>" + c[a] + "</li>";
    e += "</ul>";
    e += "<p class='" + this._cssClassDomain + "room-info' style='font-size:12px;'></p>";
    e += '<i class="fas fa-spinner fa-spin fa-2x"></i>';
    e += "</div>";
    if (b) {
        e += "<div class='" + this._cssClassDomain + "dlg-footer modal-footer " + this._cssClassDomain + "center p-2'>";
        for (a = 0; a < b.length; a++)
            c = b[a],
            e += "<button type='button' onclick='" + c.func + '("' + this._idLoadingDialog + "\");' class='" + this._cssClassDomain + "mini btn btn-outline-primary btn-sm" + (c.classes ? " " + c.classes : "") + "' style='font-size:13px;margin:2px 2px;'>" + c.label + "</button>";
...
function CMain(a) {
    ...
    this._loadImages = function() {
        ...
        s_oSpriteLibrary.addSprite("cards", "./sprites/cards.jpg");
